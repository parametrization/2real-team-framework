...
- [Framework extraction state](project_framework_extraction_state.md) — baseline: released v0.10.2 (Phase 6 W10 "harden the installer" — deferred W8 follow-on: S1 #238 amend path reconciles [not unions] config hook module-lists onto canonical so upgrade-over-diverged drops stale entries; oracle `m_config_module_lists_complete` was failing because keep-&-amend `write_config(force=False)` skipped an existing config [fixes #162, PR#240, Paloma→Nia+Tariq]; S2 #239 real-repo provisioner hardening 4/5 — partial-clone fingerprint-on-failure, de-hardcoded fixtures, merge-not-replace `--real-config`, nested-child mkdir(parents=True); item 4→#244 [fixes #155, PR#241, Ibrahim→Paloma+Tariq]; 2 PRs/0 CR/809 tests, file-disjoint, all 4 verdicts clean first-pass w/ real mutation probes; Tariq 5→5 [reserved, both-PR QA], Nia 4→4 [5-ready, review-only — flagship-caliber 3 of 4 waves, rotation tension], Paloma/Ibrahim 4→4; **✅ both W14 incidents designed out [distinct names+isolation→no collision; 12/12 CI-green confirmed→no red-merge]**; new debt #242/#243), OIDC both registries; deferred: #244 (provisioner item 4), #234 node RNG, rulesets branch-protection probe, more #102 P2, #110. Read first.
