...
|---|---|---|---|
| Per-commit `-c` identity from the roster ([commits.md](commits.md)) | `validate_commit_identity` | PreToolUse (Bash) | Blocks `git commit` without `-c user.name`/`-c user.email` matching `.claude/team/roster.json` (opt-in via `identity.enforce`) |
...
| No API reviews / no self-review ([issues.md](issues.md)) | `block_gh_pr_review` | PreToolUse (Bash) | Blocks a raw `gh pr review` (shared-user API approvals always fail) and a verdict-comment whose `Requestor:` (reviewer) equals its `Requestee:` (PR author) — a self-review. Reuses `validate_review_comment_format`'s parsing; fail-open on every other shape so it never bricks the team's own reviewing (#196) |
| PR body must not silently auto-close an issue it only quotes ([pull-requests.md](pull-requests.md)) | `validate_pr_body` | PreToolUse (Bash) | Blocks `gh pr create` / `gh pr edit` whose PR **body** carries a GitHub closing keyword (`Fix`/`Fixes`/`Closes`/`Resolves` + `#N`) bound to an issue reference — GitHub auto-closes such an issue on merge with **no trace in `git log`** (how PR #299 silently closed #296). LOCAL, stdlib-only, rate-limit-immune: reads the body from `--body`/`-b`/`--body-file`/`-F`. Markdown code spans/fences are masked first, so backticked `` `#N` `` and prose ("Address #N", "per #N") do NOT trip — smarter than a naive grep. Closing keywords belong in the COMMIT MESSAGE (visible/greppable), never the body. Fail-open; audited override `PR_BODY_CLOSING_KEYWORD_EXCEPTION="<rationale>"` (#304) |
