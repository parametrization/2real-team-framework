#!/usr/bin/env python3
"""PreToolUse hook: Block --no-verify on git commit/push.

Detects `--no-verify` or `-n` (short form) on git commit / push commands and
blocks them — project policy requires the pre-commit gate to run. Gated on
`scm.allow_force`: when force is allowed, this guard returns None (allow).
...
Does NOT match:
    - Heredoc / --body / --body-file argument bodies that mention
      "--no-verify" in prose. Tokenized via shlex so data-position
...
def _segment_has_no_verify(segment: list[str]) -> bool:
    ...
    `-n` is matched only on `git commit` (intent: bypassing the pre-commit
    gate). `git push -n` means dry-run, not bypass; we don't block that.
    """
...
def check(input_data: dict) -> dict | None:
    ...
    # Opinionated gate: if the project allows force/no-verify, do not block.
    if config(input_data).get("scm.allow_force", False):
        return None
    ...
    result = {
        ...
        "reason": (
            ...
            "This bypasses the framework's pre-commit gate, which project policy "
            "requires to run. Do not use --no-verify routinely. "
            "If you have a legitimate emergency reason, remove --no-verify and "
            "fix the underlying hook failure instead (or set scm.allow_force in "
            "the framework config to relax this guard)."
        ),
    }
    log_pretooluse_block(
        "block_no_verify", command, result["reason"], input_data=input_data
    )
    return result
