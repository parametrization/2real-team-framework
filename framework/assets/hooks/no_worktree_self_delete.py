...
Matches (one or more segments in the command, split on &&, ||, ;, |):
    ...
    - A leading `cd <dir>` IS honored when it names an absolute directory
      that exists on disk: the cwd used for the ancestry check is the
...
Fail-open: any unexpected exception returns None (allow). Matches dispatcher
convention — hook bugs must never block the user.
"""
...
def _block_reason(target: str, cwd: str) -> str:
    ...
    lines = [
        ...
        "directory and typically forces a restart.",
        "",
    ]
    if safe:
...
def check(input_data: dict) -> dict | None:
    ...
    try:
        ...
        # remove` runs. resolve_invocation_cwd applies last-cd-wins recovery:
        # it returns the target of the last `cd <absolute-existing-dir>` in the
        # command, else the stdin `cwd`, else os.getcwd().
        #
        # Recovery fixes the block/allow SAFETY verdict, and it makes it MORE
        # correct in BOTH directions:
        #   cd <wt> && git worktree remove <wt>   → resolves to <wt>  → BLOCK
        #       (the self-delete a plain stdin-cwd read can MISS when the
        #        harness cwd is anchored at the parent, not <wt>).
...
def main() -> None:
    ...
    if result.get("decision") == "block":
        ...
        log_pretooluse_block(
            "no_worktree_self_delete", command, result["reason"], input_data=input_data
        )
        sys.exit(2)
    sys.exit(0)
