...
When the dev environment's interactive shell AND the agent Bash tool run under
**zsh**, several bash-only idioms that rely on word-splitting of unquoted scalar
variables work in bash but silently misbehave under zsh. This hook nudges
authors toward zsh-safe forms. It is gated on the `shell` config key: it is a
...
- Path/glob/concat prefixes where ``$NAME`` is only PART of a word:
  ...
  a single scalar path component (no word-splitting), so it is SAFE. The
...
Block vs. warn (deliberate decision)
====================================
...
friction. The dispatcher returns ``{"decision": "allow", "systemMessage": ...}``
for any non-blocking hook result — returning ``{"systemMessage": ...}`` (no
decision key) is treated identically (decision defaults to allow).

Input Language
==============
  Fires on:      PreToolUse Bash (only when shell == "zsh")
...
# ---------------------------------------------------------------------------
# Scalar reference + standalone-word boundary (FP fix).
#
...
# path/glob-prefix false positives while keeping the bare ``$list`` /
# ``$a $b $c`` cases firing.
#
...
def _scan_for_wordsplit(command: str) -> list[tuple[str, str]]:
    ...
    No secondary safe-form filter is needed — the regex enforces both
    constraints.
    """
...
def check(input_data: dict) -> dict | None:
    ...
    No-op unless the project's configured shell is zsh. Returns None when no
    bash-ism word-splitting construct is found. Returns ``{"systemMessage": ...}``
    (advisory, never blocking) when one or more risky constructs are detected.
    """
    # Opinionated gate: only relevant when the project shell is zsh.
    if config(input_data).get("shell", "bash") != "zsh":
        return None
