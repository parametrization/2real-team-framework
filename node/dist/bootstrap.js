...
export function generateName(used) {
    for (let i = 0; i < 100; i++) {
...
export function makeEmail(first, last, prefix = "") {
...
function renderSkill(name, context) {
...
export async function bootstrap(opts) {
    ...
    if (opts.config) {
        ...
        if (yamlCfg.project_name)
            opts.projectName = yamlCfg.project_name;
        if (yamlCfg.team_size)
            opts.teamSize = yamlCfg.team_size;
        if (yamlCfg.git_email_prefix)
            emailPrefix = yamlCfg.git_email_prefix;
        if (yamlCfg.target && yamlCfg.target !== ".")
            target = resolve(yamlCfg.target);
        if (yamlCfg.skills)
            configSkills = yamlCfg.skills;
        if (yamlCfg.members)
            memberOverrides = yamlCfg.members;
    ...
    if (opts.aiPersonas) {
        ...
        const personas = await generatePersonas({ name: preset.name, description: preset.description ?? preset.name }, roleSpecs, members.length, opts.seed);
...
export function removeMember(opts) {
    const target = resolve(opts.target);
    const rosterDir = join(target, ".claude", "team", "roster");
    if (!existsSync(rosterDir)) {
    ...
    const files = readdirSync(rosterDir).filter((f) => f.includes(safeName) && !f.startsWith("_departed_"));
    if (files.length === 0) {
...
export function replaceField(content, field, value) {
    return content
        .split("\n")
        .map((line) => {
        if (line.includes(`**${field}:**`)) {
...
export function findRosterCards(rosterDir, name) {
    if (!existsSync(rosterDir))
        return [];
...
export async function randomizeMember(opts) {
    const target = resolve(opts.target);
    const rosterDir = join(target, ".claude", "team", "roster");
    const safeName = opts.name.toLowerCase().replace(/ /g, "_").replace(/-/g, "_");
    const files = readdirSync(rosterDir).filter((f) => f.includes(safeName) && !f.startsWith("_departed_"));
...
export function validateTeam(opts) {
    const target = resolve(opts.target);
    const teamDir = join(target, ".claude", "team");
    const skillsDir = join(target, ".claude", "skills");
    const errors = [];
    ...
    if (!existsSync(rosterDir)) {
        ...
        const active = readdirSync(rosterDir).filter((f) => f.endsWith(".md") && !f.startsWith("_departed_"));
        if (active.length === 0) {
...
export function showStatus(opts) {
    ...
    console.log(`${"Name".padEnd(25)} ${"Role".padEnd(20)} ${"Level".padEnd(12)} Status`);
    console.log("─".repeat(60));
